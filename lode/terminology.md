...
- **Reviewed candidate** — one bounded line-addressed edit selecting an admitted source by numeric index, an inclusive 1-based line range, exact replacement bytes, and a component-computed source and materialized result identity.
- **Proposal generation** — one exact original or corrected line-addressed edit for an admitted path index; only the latest completed generation remains promotable.
...
- **Safe terminal metadata** — optional terminal usage and a SHA-256 response-ID digest plus an allowlisted remote error code or incomplete reason; raw provider diagnostics remain outside durable state.
